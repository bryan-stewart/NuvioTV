...
class WatchStateMutationStore @Inject constructor(
    ...
    suspend fun queueProgressDeletes(keys: Collection<String>, profileId: String) {
    ...
    suspend fun pendingProgressUpserts(profileId: String): Map<String, WatchProgress> {
        val preferences = store(profileId).data.first()
        return parseProgressUpserts(preferences[progressUpsertsKey])
    }

    suspend fun pendingProgressDeletes(profileId: String): Set<String> {
    ...
    suspend fun acknowledgeProgressDeletes(keys: Collection<String>, profileId: String) {
    ...
    suspend fun queueWatchedUpserts(items: Collection<WatchedItem>, profileId: String) {
    ...
    suspend fun queueWatchedDeletes(keys: Collection<WatchedMutationKey>, profileId: String) {
        if (keys.isEmpty()) return
    ...
    suspend fun pendingWatchedUpserts(profileId: String): Map<WatchedMutationKey, WatchedItem> {
        val preferences = store(profileId).data.first()
        return parseWatchedUpserts(preferences[watchedUpsertsKey])
    }

    suspend fun pendingWatchedDeletes(profileId: String): Set<WatchedMutationKey> {
        return parseWatchedDeletes(store(profileId).data.first()[watchedDeletesKey])
    }

    suspend fun acknowledgeWatchedUpserts(items: Collection<WatchedItem>, profileId: String) {
        if (items.isEmpty()) return
        store(profileId).edit { preferences ->
            val pending = parseWatchedUpserts(preferences[watchedUpsertsKey]).toMutableMap()
            items.forEach { item ->
    ...
    suspend fun hasPendingProgressMutations(profileId: String): Boolean {
        val preferences = store(profileId).data.first()
        return !preferences[progressUpsertsKey].isNullOrEmpty() ||
            !preferences[progressDeletesKey].isNullOrEmpty()
    }

    suspend fun hasPendingWatchedMutations(profileId: String): Boolean {
        val preferences = store(profileId).data.first()
